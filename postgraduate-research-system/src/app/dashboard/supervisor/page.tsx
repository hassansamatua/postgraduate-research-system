...
import { Users, FileText, MessageSquare, CheckCircle, Download, Send, Clock, Eye, Calendar } from 'lucide-react';
...
function SupervisorDashboardContent() {
  ...
  useEffect(() => {
    const tabParam = searchParams.get('tab') as 'students' | 'documents' | 'messages' | 'defenses' | null;
    if (tabParam && ['students', 'documents', 'messages', 'defenses'].includes(tabParam)) {
  ...
  const handleScheduleDefense = async () => {
    ...
    try {
      ...
      if (res.ok) { setScheduleModal(null); await fetchAll(); }
        ...
        <div className="border-b border-gray-200">
          <nav className="flex gap-6">
            {[
              ...
              { id: 'defenses' as const, label: `Defenses (${defenses.length})` },
            ].map(tab => (
